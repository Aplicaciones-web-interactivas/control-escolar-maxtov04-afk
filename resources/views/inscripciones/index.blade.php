...
<div class="max-w-6xl mx-auto">
    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded shadow-md mb-8 transition-colors duration-300">
        <h2 class="text-xl font-bold mb-4 dark:text-white transition-colors">Inscribir Alumno a Grupo</h2>
        <form action="{{ route('inscripciones.guardar') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            @csrf
            ...
            <select name="usuario_id" class="border dark:border-gray-600 p-2 rounded w-full dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none transition-all" required>
                ...
                @foreach($alumnos as $alumno)
                    <option value="{{ $alumno->id }}">({{ $alumno->clave_institucional }}) {{ $alumno->nombre }}</option>
                @endforeach
            </select>
            ...
            <select name="grupo_id" class="border dark:border-gray-600 p-2 rounded w-full dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none transition-all" required>
                ...
                @foreach($grupos as $grupo)
                    <option value="{{ $grupo->id }}">{{ $grupo->nombre }} - {{ $grupo->horario->materia->nombre }} (Prof. {{ $grupo->horario->profesor->nombre }})</option>
                @endforeach
            </select>
            
            <button type="submit" class="bg-blue-600 dark:bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-700 dark:hover:bg-blue-600 transition-colors h-10.5 self-end">Inscribir</button>
        </form>
    </div>
            ...
            <tbody>
                ...
                <tr class="border-t dark:border-gray-700 transition-colors">
                    ...
                    <td class="p-3 flex gap-2">
                        <a href="{{ route('inscripciones.editar', $inscripcion->id) }}" class="text-blue-600 dark:text-blue-400 font-bold hover:text-blue-800 dark:hover:text-blue-300 transition-colors">Editar</a>
                        
                        <form id="form-delete-{{ $inscripcion->id }}" action="{{ route('inscripciones.eliminar', $inscripcion->id) }}" method="POST">
                            @csrf @method('DELETE')
                            <button type="button" onclick="abrirModal({{ $inscripcion->id }})" class="text-red-600 dark:text-red-400 font-bold hover:text-red-800 dark:hover:text-red-300 transition-colors">Dar de baja</button>
                        </form>
                    </td>
                </tr>
                @endforeach
...
<div id="modal-delete" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm transition-opacity">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full p-6 mx-4 transform transition-all">
        <div class="flex items-center gap-3 mb-4">
            ...
            <h3 class="text-lg font-bold text-gray-900 dark:text-white">Eliminar Inscripción</h3>
        </div>
        <p class="text-sm text-gray-600 dark:text-gray-300 mb-6">
            ¿Estás seguro de que deseas dar de baja a este alumno del grupo?
        </p>
        <div class="flex justify-end gap-3">
            ...
            <button type="button" onclick="confirmarEliminacion()" class="px-4 py-2 bg-red-600 text-white rounded font-bold hover:bg-red-700 transition-colors">Sí, dar de baja</button>
        </div>
    </div>
</div>
